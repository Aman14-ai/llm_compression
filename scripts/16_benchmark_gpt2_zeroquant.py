...
OUTPUT_JSON = RESULTS_DIR / "gpt2_zeroquant_inference_benchmark_bs8_ep3_wd001_warmup500.json"
...
def is_gpt2_conv1d(module: nn.Module) -> bool:
    return module.__class__.__name__ == "Conv1D" and hasattr(module, "weight") and hasattr(module, "bias")
...
def quantize_linear_weight_per_output_channel(weight: torch.Tensor, bits: int):
    """
    For nn.Linear:
        weight shape = [out_features, in_features]
        per-output-channel scale is along rows.
    """
    qmax = (2 ** (bits - 1)) - 1
...
def quantize_conv1d_weight_per_output_channel(weight: torch.Tensor, bits: int):
    """
    For Hugging Face GPT2 Conv1D:
        weight shape = [in_features, out_features]
        output channels are columns.
    """
    qmax = (2 ** (bits - 1)) - 1
    w = weight.detach().float().cpu()

    max_abs = w.abs().amax(dim=0, keepdim=True)
    scale = torch.clamp(max_abs / qmax, min=1e-8)
...
class QuantizedLinear(nn.Module):
    ...
    def from_float(cls, linear: nn.Linear, weight_bits: int, activation_bits: int | None):
        qweight, scale = quantize_linear_weight_per_output_channel(
            linear.weight,
            bits=weight_bits,
        )
...
class QuantizedGPT2Conv1D(nn.Module):
    """
    Quantized replacement for Hugging Face GPT2 Conv1D.

    GPT2 Conv1D behaves like:
        y = x @ W + b
    ...
    @classmethod
    def from_float(cls, conv1d: nn.Module, weight_bits: int, activation_bits: int | None):
        qweight, scale = quantize_conv1d_weight_per_output_channel(
            conv1d.weight,
            bits=weight_bits,
        )
...
def replace_layers_with_zeroquant(module: nn.Module, prefix: str = ""):
    replaced = []
    ...
    for child_name, child in list(module.named_children()):
        ...
        if isinstance(child, nn.Linear):
        ...
        elif is_gpt2_conv1d(child):
            qconv = QuantizedGPT2Conv1D.from_float(
                child,
                weight_bits=WEIGHT_BITS,
                activation_bits=ACTIVATION_BITS,
            )
            ...
            replaced.extend(replace_layers_with_zeroquant(child, prefix=full_name))
...
def benchmark(model, dataloader, device):
    ...
    for batch in tqdm(dataloader, desc="Benchmarking GPT2 ZeroQuant inference"):
        batch = move_batch_to_device(batch, device)
        ...
        batch_latencies_ms.append((batch_end - batch_start) * 1000)
    ...
    result = {
        ...
        "dataset": "AG News",
        "method": "ZeroQuant",
        "method_description": "ZeroQuant-style INT8 weight quantization plus INT8 activation fake quantization for GPT2 Conv1D and Linear layers.",
        "weight_bits": WEIGHT_BITS,
        "activation_bits": ACTIVATION_BITS,
        "device": device,
        "batch_size": BATCH_SIZE,
...
def main():
    ...
    print("=" * 100)
    print("GPT2 AG News ZeroQuant-style benchmark")
    print("=" * 100)
    ...
    print("\nLoading GPT2 baseline model on CPU:", MODEL_DIR)
    model = AutoModelForSequenceClassification.from_pretrained(str(MODEL_DIR))

    if getattr(model.config, "pad_token_id", None) is None:
        model.config.pad_token_id = GPT2_PAD_TOKEN_ID
    ...
    print("\nApplying GPT2 ZeroQuant-style quantization...")
    replaced_layers = replace_layers_with_zeroquant(model)
    ...
    print("\nMoving quantized GPT2 model to device...")
    model.to(DEVICE)
    model.eval()
